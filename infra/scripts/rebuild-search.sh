...
stop_failed_rebuild_on_exit() {
  ...
  if [ "$status" -ne 0 ] && [ "$rebuild_barrier_entered" = true ] && [ "$rebuild_complete" = false ]; then
    if ! compose --profile account-lifecycle stop account-erasure-worker search-projection-worker api >/dev/null 2>&1; then
      printf 'ERROR: failed search rebuild could not stop every writer; inspect immediately.\n' >&2
    else
      printf 'ERROR: failed search rebuild left every writer stopped for explicit recovery.\n' >&2
    fi
  fi
  exit "$status"
}
...
# The rebuild clears the satisfied projection log after indexing. Stop every
# canonical/search writer so the database snapshot and log retirement are one
# stable operational epoch.
...
if service_is_active api || service_is_active search-projection-worker || profiled_service_is_active account-lifecycle account-erasure-worker; then
  die "API, search-projection-worker, and account-erasure-worker must stop before rebuilding search"
fi
