...
readonly EXPECTED_CSP="default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'self'; form-action 'self'; script-src 'self' https://*.clerk.accounts.dev https://clerk.${DOMAIN}; script-src-elem 'self' 'unsafe-inline' https://*.clerk.accounts.dev https://clerk.${DOMAIN}; script-src-attr 'none'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https://img.clerk.com; font-src 'self' data:; connect-src 'self' https://*.clerk.accounts.dev https://clerk.${DOMAIN} https://img.clerk.com; worker-src 'self' blob:; frame-src 'self' blob: https://challenges.cloudflare.com; manifest-src 'self'; media-src 'none'"
