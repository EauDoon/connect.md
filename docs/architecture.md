...
`POST /v1/ingest` accepts bounded PDF, DOCX, Markdown, or plain-text uploads. Markdown and plain text use the direct UTF-8 path. PDF and DOCX conversion is enabled only when `CONNECTMD_INGEST_JOBS_PATH` is configured and the no-network worker has a fresh heartbeat; otherwise binary ingestion fails closed with a structured 503 and no draft. The worker gives each conversion its own process and enforces `conversion_timeout_seconds`; the API cleans its job files after its bounded wait, while worker cleanup and recovery handle claimed or stale worker artifacts. `/v1/capabilities` reports worker configuration and heartbeat liveness separately, so configuration is not presented as runtime readiness. Binary conversion first uses `markitdown`, with local `python-docx` plain-text extraction as the DOCX fallback and local Poppler/Tesseract OCR as the PDF fallback. OCR processes at most 30 pages sequentially, scales each grayscale raster to at most 2400 pixels per side, bounds raster and cumulative normalized UTF-8 output size, independently caps raw OCR output at 16 MiB per page, and enforces a 45-second OCR budget inside the worker hard deadline. OCR uses the image's installed English language data and never downloads models; unsupported encrypted, excessive-page, oversized, or failed OCR inputs return no draft. These bounds apply to the OCR fallback, while text PDFs retain the primary extraction path. DOCX fallback preserves ordered paragraphs and table rows, including nested cell text, plus active headers and footers, respecting first-page and even-page flags and section inheritance. Each active definition is extracted once. Entirely empty table rows are omitted, while empty columns around content and internal whitespace remain. UTF-8 limits count the final trimmed text. A source-body content check prevents primary-converter table scaffolding from accepting a blank DOCX and lets header/footer-only text reach the fallback; literal punctuation and primary-supported rich content retain primary conversion. The fallback does not reconstruct page layout, font styling, or image contents. The unused `unstructured`, spaCy model-download, inference, and CUDA dependency stack is excluded. After conversion, all paths perform deterministic normalization into the requested schema and strict validation. The MVP is deterministic and does not require or invoke an LLM. Ingestion returns a draft plus warnings and provenance; it does not publish automatically.
