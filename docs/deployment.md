...
Set at least `POSTGRES_PASSWORD`, all six `CONNECTMD_*_DB_PASSWORD` values shown in `.env.example`, `MEILI_MASTER_KEY`, `CONNECTMD_EXACT_SEARCH_CURSOR_KEYRING`, `CONNECTMD_API_KEY_PEPPER`, `CONNECTMD_DOMAIN`, `CONNECTMD_PUBLIC_BASE_URL`, `NEXT_PUBLIC_SITE_URL`, and `ACME_EMAIL`. Generate every PostgreSQL password independently with `openssl rand -hex 32`; production rejects non-hexadecimal, short, reused values and any `POSTGRES_USER` other than the operator-only `postgres` bootstrap identity. `CONNECTMD_PUBLIC_BASE_URL` must be the backend's canonical `CONNECTMD_DOMAIN` HTTPS origin; `NEXT_PUBLIC_SITE_URL` may instead be a distinct canonical frontend origin such as a Vercel production domain. Set `NEXT_PUBLIC_API_BASE_URL` to the backend origin for that split deployment. These values prevent reverse-proxy internals from leaking into discovery documents, OAuth metadata, A2A cards, canonical links, or JSON-LD.

For authenticated human workspaces, also replace `CONNECTMD_CLERK_JWKS_URL`, `CONNECTMD_CLERK_ISSUER`, `CONNECTMD_CLERK_AUTHORIZED_PARTIES`, `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, and `CLERK_SECRET_KEY` together. Leave `CONNECTMD_CLERK_AUDIENCE` blank for normal Clerk session tokens; set it only when the frontend deliberately requests a Clerk JWT template with the same `aud` claim. For an explicit public-only release, blank the JWKS URL, issuer, audience, publishable key, and secret key, and set `CONNECTMD_CLERK_AUTHORIZED_PARTIES=[]`; private routes then remain unavailable and token verification fails closed. Partial Clerk configuration is rejected, and account lifecycle requires complete API and frontend Clerk configuration. Database passwords, `MEILI_MASTER_KEY`, runtime Meilisearch keys, the exact-search cursor keyring, `CONNECTMD_API_KEY_PEPPER`, and `CLERK_SECRET_KEY` are server-only and must never be set in `NEXT_PUBLIC_*` variables. When enabled, `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` must use the `pk_test_` or `pk_live_` family followed by standard Base64 encoding of an ASCII dotted Clerk instance hostname ending in exactly one `$`; `CLERK_SECRET_KEY` must be a real `sk_test_...` or `sk_live_...` secret with at least 16 URL-safe suffix characters. Malformed values are rejected before release acceptance without being echoed. Meilisearch runs in `production` mode and each key must be at least 16 bytes.
