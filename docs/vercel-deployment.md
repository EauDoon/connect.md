...
| `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` | Every Vercel environment that intentionally enables Clerk | Build and browser | The real Clerk publishable key. Omit it for a public-only deployment; private routes then remain unavailable and Clerk origins are excluded from the CSP. When configured, the CSP derives the Clerk Frontend API origin from this key, so do not replace it with a site-derived hostname. |
...
In Vercel Project Settings, enable **Automatically expose System Environment Variables**. The frontend uses the exposed `VERCEL` value to activate the Vercel security headers through `next.config.ts`; `VERCEL_ENV` identifies the selected deployment environment. A Vercel deployment with those system variables disabled can still compile but must fail live acceptance because the required headers will be absent. A Vercel production build fails closed if either public origin is missing or malformed, and rejects any configured malformed Clerk key. Omitting both Clerk keys is the explicit public-only mode: private routes remain unavailable and the CSP grants no Clerk origins. Docker builds do not emit this second policy, so the existing Nginx security header and CSP boundary remains authoritative there.
