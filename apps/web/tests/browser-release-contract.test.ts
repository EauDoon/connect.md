...
function exactThirtyPassPlaywrightReceipt() {
  return {
    config: {},
    stats: {
...
describe("production browser release gate", () => {
  ...
  it("accepts only the exact thirty-pass Playwright JSON receipt", () => {
    const receipt = exactThirtyPassPlaywrightReceipt();
    ...
    const redistributed = exactThirtyPassPlaywrightReceipt();
    ...
    const failedSpec = exactThirtyPassPlaywrightReceipt();
  ...
  it("reports bounded Playwright status without exposing child output", () => {
    const passing = summarizePlaywrightResult({
      code: 0,
      signal: null,
      stdout: JSON.stringify(exactThirtyPassPlaywrightReceipt()),
    });
    expect(passing).toContain("expected=30");
    expect(passing).toContain("failed=0");
    expect(passing).toContain("skipped=0");

    const failed = exactThirtyPassPlaywrightReceipt();
    ...
    const untrustedLocation = exactThirtyPassPlaywrightReceipt();
  ...
  it("keeps the harness loopback-only, read-only, and fail-closed", () => {
    ...
    expect(harness).toContain("EXPECTED_PLAYWRIGHT_TESTS = 30");
