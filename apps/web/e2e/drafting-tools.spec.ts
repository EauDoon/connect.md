...
test("search options protect metadata and stale replacement reviews", async ({ page }) => {
  const writes: string[] = [];
  page.on("request", (request) => { if (request.method() !== "GET") writes.push(request.url()); });
  await page.goto("/md");
  await page.getByRole("radio", { name: "Plain-text editor", exact: true }).check();
  const source = page.getByRole("textbox", { name: "Canonical Markdown source" });
  const draft = "---\nname: Cat\n---\n# Cat\nCAT scatter caté cat_\n";
  await source.fill(draft);
  await page.getByText("Find and replace source text", { exact: true }).click();
  await page.getByLabel("Find text", { exact: true }).fill("cat");
  await page.getByLabel("Match case", { exact: true }).uncheck();
  await page.getByLabel("Whole words", { exact: true }).check();
  await page.getByLabel("Body only", { exact: true }).check();
  await page.getByLabel("Replace with", { exact: true }).fill("Dog");
  await page.getByRole("button", { name: "Review all replacements", exact: true }).click();
  await expect(page.getByLabel("After replacement", { exact: true })).toContainText("# Dog");
  await source.fill(draft + "New work\n");
  await expect(page.getByRole("button", { name: "Apply reviewed replacement", exact: true })).toBeDisabled();
  await page.getByRole("button", { name: "Review all replacements", exact: true }).click();
  await page.getByLabel("Replace with", { exact: true }).fill("Fox");
  await expect(page.getByRole("button", { name: "Apply reviewed replacement", exact: true })).toBeDisabled();
  await page.getByRole("button", { name: "Review all replacements", exact: true }).click();
  await page.getByRole("button", { name: "Apply reviewed replacement", exact: true }).click();
  await expect(source).toHaveValue("---\nname: Cat\n---\n# Fox\nFox scatter caté cat_\nNew work\n");
  expect(writes).toEqual([]);
  expect(await page.evaluate(() => [localStorage.length, sessionStorage.length])).toEqual([0, 0]);
});

test("outline navigation and reviewed excerpts work on narrow screens", async ({ page }) => {
  ...
  await source.fill(draft);
  await page.getByText("Document outline and length", { exact: true }).click();
  await page.getByLabel("Source line", { exact: true }).fill("999");
  await page.getByRole("button", { name: "Go to line", exact: true }).click();
  await expect(page.getByRole("alert").filter({ hasText: "existing source line" })).toBeVisible();
  await page.getByLabel("Source line", { exact: true }).fill("7");
  await page.getByRole("button", { name: "Go to line", exact: true }).focus();
  await page.keyboard.press("Enter");
  await expect(source).toBeFocused();
  expect(await source.evaluate((element: HTMLTextAreaElement) => element.value.slice(element.selectionStart, element.selectionEnd))).toBe("Facts");
  await page.getByLabel("Filter headings", { exact: true }).fill("missing");
  await expect(page.getByRole("navigation", { name: "Document outline" })).not.toContainText("Work");
  await page.getByLabel("Filter headings", { exact: true }).fill("work");
  await page.getByRole("button", { name: "Review excerpt Work at line 6", exact: true }).click();
  await source.fill(draft + "Later\n");
  await expect(page.getByRole("button", { name: "Download section excerpt", exact: true })).toBeDisabled();
  await page.getByRole("button", { name: "Review excerpt Work at line 6", exact: true }).click();
  const pending = page.waitForEvent("download");
  await page.getByRole("button", { name: "Download section excerpt", exact: true }).click();
  const file = await pending;
  expect(file.suggestedFilename()).toBe("excerpt-work.md");
  const chunks: Buffer[] = [];
  for await (const chunk of (await file.createReadStream())!) chunks.push(Buffer.from(chunk));
  expect(Buffer.concat(chunks).toString()).toBe("## Work\nFacts\n### Detail\nMore\n");
  await expect(source).toHaveValue(draft + "Later\n");
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  await page.screenshot({ path: test.info().outputPath("section-excerpt-mobile.png"), fullPage: true });
  await page.setViewportSize({ width: 1440, height: 1000 });
  await page.screenshot({ path: test.info().outputPath("section-excerpt-desktop.png"), fullPage: true });
});
