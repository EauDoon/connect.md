  ...
  return (
    <section aria-labelledby="validation-title" className="rounded-2xl border border-white/10 bg-black/20 p-4">
      <div className="flex min-w-0 flex-wrap items-center justify-between gap-4">
        ...
        <span className={errors.length ? "min-w-0 break-words text-xs font-medium text-red-200" : "min-w-0 break-words text-xs font-medium text-acid"}>{errors.length ? `${errors.length} issue${errors.length === 1 ? "" : "s"}` : "Ready for API validation"}</span>
      </div>
