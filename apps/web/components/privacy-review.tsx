...
export function PrivacyReview() {
  ...
  return <details className="mt-4 border-t border-white/10 pt-3">
    <summary className="min-h-11 cursor-pointer py-3 text-sm font-semibold text-white">Sharing review ({review.findings.length}{review.limited ? "+" : ""})</summary>
    ...
    <ul className="mt-2 space-y-2">{review.findings.map((finding) => <li key={`${finding.code}-${finding.line}`} className="text-xs leading-5 text-amber-100"><ReviewSourceLink line={finding.line} />: {finding.message}</li>)}</ul>
