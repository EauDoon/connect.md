...
import { ReviewSourceLink } from "@/components/review-source-link";
...
export function WritingReview() {
  ...
  return <details className="mt-4 border-t border-white/10 pt-3">
    ...
    <p className="mt-2 text-xs leading-5 text-mist">Suggestions for unfinished text, empty sections, repetition, heading hierarchy, simple inline links, and readability. Link checks never open destinations or verify availability. These checks do not verify facts, judge your experience, or affect schema validation.</p>
    ...
    <ol className="mt-2 space-y-2">{review.findings.map((finding, index) => <li key={`${finding.code}-${finding.line}-${index}`} className="text-xs leading-5 text-mist"><ReviewSourceLink line={finding.line} />: {finding.message}</li>)}</ol>
