...
export function DocumentOutline({ markdown, onSelect }: { markdown: string; onSelect: (offset: number) => void }) {
  ...
  return <details className="mb-4 rounded-xl border border-white/10 p-3">
    <summary className="cursor-pointer text-sm font-semibold text-white">Document outline and length</summary>
    ...
    <nav aria-label="Document outline" className="mt-2 max-h-60 overflow-auto">
      <ol>{metrics.headings.slice(0, 60).map((heading) => <li key={heading.start}>
        <button type="button" onClick={() => onSelect(heading.start)} aria-label={`Edit ${heading.text.slice(0, 120)} at line ${heading.line}`} className="min-h-9 w-full break-words rounded-lg px-2 py-1 text-left text-xs text-mist hover:bg-white/5 hover:text-white">
