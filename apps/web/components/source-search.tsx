...
export function SourceSearch({ markdown, onChange, onSelect }: { markdown: string; onChange: (value: string) => void; onSelect: (start: number, end: number) => void }) {
  ...
  const search = useMemo(() => {
    try { return { ...findSourceMatches(markdown, query), error: "" }; }
  ...
  function replace(target: number | "all") {
    try {
      ...
      onChange(replaceSourceMatches(markdown, query, replacement, target));
  ...
  return <details className="mb-4 border-b border-white/10 pb-3">
    ...
    <p className="text-xs leading-5 text-mist">Case-sensitive, literal text. Includes frontmatter. Selecting a match opens the plain-text editor; no regular expressions are used.</p>
