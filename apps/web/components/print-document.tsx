...
function PrintDialog({ markdown, onClose }: { markdown: string; onClose: () => void }) {
  ...
  return createPortal(<dialog ref={dialogRef} onClose={onClose} aria-labelledby="print-document-title" className="draft-print-dialog w-[calc(100%_-_2rem)] max-w-3xl rounded-2xl bg-white p-6 text-slate-950 shadow-2xl backdrop:bg-black/70">
    <div className="draft-print-controls mb-6 border-b border-slate-200 pb-4">
      ...
      <div className="mt-4 flex flex-wrap gap-3"><Button onClick={() => window.print()}>Print document</Button><button type="button" autoFocus onClick={() => dialogRef.current?.close()} className="min-h-11 rounded-full border border-slate-400 px-5 text-sm font-semibold">Close print preview</button></div>
    </div>
