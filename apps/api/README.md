...
All settings use the `CONNECTMD_` prefix. `CONNECTMD_DATABASE_URL` must be an async SQLAlchemy URL such as `postgresql+asyncpg://connectmd:...@postgres:5432/connectmd`. Production Clerk verification requires `CONNECTMD_CLERK_JWKS_URL`, `CONNECTMD_CLERK_ISSUER`, and `CONNECTMD_CLERK_AUTHORIZED_PARTIES` (JSON list); `CONNECTMD_CLERK_AUDIENCE` is optional. JWT authentication is Bearer-only; the API does not accept Clerk session cookies. Same-origin deployment needs no CORS setting; a separate browser origin must be listed exactly in `CONNECTMD_CORS_ORIGINS`.
